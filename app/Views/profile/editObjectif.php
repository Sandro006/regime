...
<body class="font-body-md text-on-surface">
    ...
    <main class="max-w-2xl mx-auto px-container-margin py-xl pb-32">
        ...
        <section class="bg-surface-container-lowest p-lg rounded-xl shadow-sm border border-surface-variant/20">
            <div class="mb-xl">
                <h2 class="font-display-md text-display-md text-on-surface mb-sm">Change Your Goal</h2>
                <p class="text-on-surface-variant font-body-md">Mettez à jour votre objectif de santé</p>
            </div>
            ...
            <form action="/profile/doEditObjectif" method="POST" class="space-y-lg">
                <?php csrf_field(); ?>

                <!-- Objectifs Selection -->
                <div>
                    <label class="font-label-caps text-label-caps text-on-surface-variant mb-sm block">Sélectionnez votre objectif</label>
                    <div class="space-y-sm">
                        <?php if (isset($objectifs) && !empty($objectifs)): ?>
                            <?php foreach ($objectifs as $objectif): ?>
                                <label class="flex items-center p-md border-2 border-outline-variant rounded-lg cursor-pointer hover:bg-surface-container-lowest transition-all <?php echo (isset($currentObjectif) && $currentObjectif == $objectif['id']) ? 'border-primary bg-primary/5' : ''; ?>">
                                    <input 
                                        type="radio" 
                                        name="objectif_id" 
                                        value="<?php echo $objectif['id']; ?>"
                                        <?php echo (isset($currentObjectif) && $currentObjectif == $objectif['id']) ? 'checked' : ''; ?>
                                        class="w-4 h-4 accent-primary cursor-pointer"
                                        required
                                    />
                                    <span class="ml-md flex-1">
                                        <span class="font-label-caps text-label-caps text-on-surface block font-bold"><?php echo htmlspecialchars($objectif['objectif']); ?></span>
                                        <span class="font-body-md text-body-md text-on-surface-variant"><?php echo htmlspecialchars($objectif['description']); ?></span>
                                    </span>
